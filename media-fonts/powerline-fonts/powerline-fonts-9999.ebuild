...
LICENSE="
	ttf-3270? ( BSD )
	ttf-anonymouspro? ( OFL-1.1 )
	ttf-arimo? ( Apache-2.0 )
	ttf-cousine? ( Apache-2.0 )
	ttf-d2coding? ( OFL-1.1 )
	ttf-dejavusansmono? ( BitstreamVera )
	otf-droidsansmono? ( Apache-2.0 )
	ttf-droidsansmonodotted? ( Apache-2.0 )
	ttf-droidsansmonoslashed? ( Apache-2.0 )
	otf-firamono? ( OFL-1.1 )
	ttf-gomono? ( GPL-2 )
	ttf-hack? ( BitstreamVera )
	otf-inconsolata-g? ( OFL )
	otf-inconsolata? ( OFL )
	otf-inconsolatadz? ( OFL-1.0 )
	ttf-liberationmono? ( OFL-1.1 )
	ttf-meslodotted? ( Apache-2.0 )
	ttf-mesloslashed? ( Apache-2.0 )
	ttf-monofur? ( monofur )
	ttf-notomono? ( OFL-1.1 )
	ttf-novamono? ( OFL-1.1 )
	ttf-profont? ( MIT )
	ttf-robotomono? ( Apache-2.0 )
	otf-sourcecodepro? ( OFL-1.1 )
	ttf-spacemono? ( OFL-1.1 )
	ttf-symbolneu? ( Apache-2.0 )
	pcf-terminus? ( OFL-1.1 )
	psf-terminus? ( OFL-1.1 )
	ttf-tinos? ( Apache-2.0 )
	ttf-ubuntumono? ( UbuntuFontLicense-1.0 )"
SLOT="0"
...
IUSE_FLAGS=(
	...
	ttf-dejavusansmono
	otf-droidsansmono
	ttf-droidsansmonodotted
	ttf-droidsansmonoslashed
	otf-firamono
	ttf-gomono
	ttf-hack
	otf-inconsolata-g
...
src_install() {
	...
	src_install_font() {
		...
		flag_name="${filetype}"-"${dirname,,}"
		flag_name="${flag_name// /}"

		if use "${flag_name}"; then
			mv "${dirname}"/*.${filetype} "${FONT_S}"
			font_filetypes[${filetype}]=
			[ -f "${dirname}"/README.rst ] && newdoc "${dirname}"/README.rst "README_${dirname// /}.rst"
		fi
	}
